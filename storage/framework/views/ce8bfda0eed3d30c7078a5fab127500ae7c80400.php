          ...
          <div class="table-responsive">
            <table id="PenaltiesTable" class="table table-striped">
              ...
              <tbody>
                ...
                <tr id='Penalty<?php echo e($Penalty->PK_PenaltyID); ?>'>
					              <td><?php echo e($Penalty->PK_PenaltyID); ?></td>
              				  <td><?php echo e($Penalty->Penalty_Name); ?></td>
					              <td><?php echo e($Penalty->Penalty_Description); ?></td>

					          <td>
                        <button id="btnDelete" style = 'background-color:red;' type='button' class='btn btn-primary' data-toggle='button' aria-pressed='false' data-id='<?php echo e($Penalty->PK_PenaltyID); ?>' data-name='<?php echo e($Penalty->Penalty_Name); ?>' data-description='<?php echo e($Penalty->Penalty_Description); ?>'>Delete</button>
                        <button id="btnEdit" style = 'background-color:green;margin-right:5px;' type='button' class='btn btn-primary' data-id='<?php echo e($Penalty->PK_PenaltyID); ?>' data-name='<?php echo e($Penalty->Penalty_Name); ?>' data-description='<?php echo e($Penalty->Penalty_Description); ?>'>Edit</button>
                    </td>
                </tr>
...
<script type="text/javascript">
  $(document).ready(function(){
      ...
      $('.modal-footer').on('click','#SubmitAdd',function(){
        $.ajax({
          ...
          success: function(response){
            ...
            $('#PenaltiesTable').prepend("<tr id='Penalty" + response.PK_PenaltyID + "'><td> " + response.PK_PenaltyID + "</td><td>" + response.Penalty_Name + "</td><td>" + response.Penalty_Description  + "</td><td><button id='btnDelete' style = 'background-color:red;' type='button' class='btn btn-primary' data-toggle='button' aria-pressed='false' data-id='" + response.PK_PenaltyID + "' data-name='" + response.Penalty_Name + "' data-description='" + response.Penalty_Description + "'>Delete</button><button id='btnEdit' style = 'background-color:green;margin-right:5px;' type='button' class='btn btn-primary' data-id='" + response.PK_PenaltyID + "' data-name='" + response.Penalty_Name + "' data-description='" + response.Penalty_Description + "'>Edit</button></td></tr>");
          }
        });
      });
        ...
        $('.modal-footer').on('click','#SubmitEdit',function(){
          $.ajax({
            ...
            success: function(response){
              ...
              $('#Penalty' + response.PK_PenaltyID).replaceWith("<tr id='Penalty" + response.PK_PenaltyID + "'><td> " + response.PK_PenaltyID + "</td><td>" + response.Penalty_Name + "</td><td>" + response.Penalty_Description  + "</td><td><button id='btnDelete' style = 'background-color:red;' type='button' class='btn btn-primary' data-toggle='button' aria-pressed='false' data-id='" + response.PK_PenaltyID + "' data-name='" + response.Penalty_Name + "' data-description='" + response.Penalty_Description + "'>Delete</button><button id='btnEdit' style = 'background-color:green;margin-right:5px;' type='button' class='btn btn-primary' data-id='" + response.PK_PenaltyID + "' data-name='" + response.Penalty_Name + "' data-description='" + response.Penalty_Description + "'>Edit</button></td></tr>");
      ...
      $('.modal-footer').on('click','#SubmitDelete',function(){
                ...
                success: function(response){
                  toastr.error('Successfully Deleted Penalty','Delete Alert',{timeOut:5000});
